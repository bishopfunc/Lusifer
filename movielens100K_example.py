...
def load_data():
    ...
    if os.path.exists(processed_ratings_file):
        ...
        #                              encoding='latin-1')
        rating_test_df = pd.read_csv(
            f"Baseline/Librecommender_baseline/testset_df_{data}.csv"
        )

    # loading ratings: Train set
    rating_df = pd.read_csv(
        f"Samples/Data/{data}/u1.base",
        sep="\t",
...
def compare_ratings(user_id, llm_ratings, rating_test_df):
    predicted_ratings = pd.DataFrame.from_dict(
        llm_ratings, orient="index", columns=["predicted"]
    ).astype(float)
    actual_ratings = (
        rating_test_df[rating_test_df["user_id"] == user_id][["movie_id", "rating"]]
        .set_index("movie_id")
        .astype(float)
    )
    ...
    precision = precision_score(
        comparison["actual"], comparison["predicted"], average="micro"
    )
    recall = recall_score(
        comparison["actual"], comparison["predicted"], average="micro"
    )
...
def evaluate_result(dataframe):
    ...
    spearman_corr, spearman_pvalue = spearmanr(
        dataframe["rating"], dataframe["simulated_ratings"]
    )

    # Cohen's Kappa (Quadratic Weighted)
    kappa = cohen_kappa_score(
        dataframe["rating"], dataframe["simulated_ratings"], weights="quadratic"
    )
    ...
    print(
        f"Spearman's Rank Correlation Coefficient: {spearman_corr:.4f} (p-value: {spearman_pvalue:.4f})"
    )
    ...
    print(
        f"Off by More Than 1 Level Percentage: {off_by_more_than_1_percentage:.2f}%\n"
    )
...
if __name__ == "__main__":
    ...
    for user_id in tqdm(
        user_ids, desc="Processing users and generating summary profile"
    ):
        if users_df.loc[users_df["user_id"] == user_id, "user_profile"].any():
            ...
            user_profile = lusifer.generate_user_profile(
                user_id, recent_items_to_consider=40
            )
    ...
    for user_id in tqdm(user_ids, desc="Generating simulated ratings"):
        ...
        user_profile = users_df.loc[
            users_df["user_id"] == user_id, "user_profile"
        ].values[0]
        ...
        if not missing_ratings.empty:
            ...
            # generate rating
            llm_ratings = lusifer.rate_new_items(
                user_profile, last_N_movies, missing_ratings
            )

            # Assigning the ratings to the movies
            for movie_id, rating in llm_ratings.items():
                rating_test_df.loc[
                    (rating_test_df["user_id"] == user_id)
                    & (rating_test_df["movie_id"] == movie_id),
                    "simulated_ratings",
                ] = rating
    ...
    total_prompt_tokens = (
        lusifer.total_tokens["prompt_tokens"]
        + lusifer.total_tokens["completion_tokens"]
    )
    total_cost = ((lusifer.total_tokens["prompt_tokens"] / 1000000) * 0.15) + (
        (lusifer.total_tokens["completion_tokens"] / 1000000) * 0.6
    ...
    print(
        f"Total Tokens: {lusifer.total_tokens['prompt_tokens'] + lusifer.total_tokens['completion_tokens']}"
    )
